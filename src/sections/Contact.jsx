export default function Contact() {
  const whatsappContacts = [
    { name: "Ventas", number: "+573106564709" },
    { name: "Gerencia / Proyectos", number: "+573132618887" },
    { name: "Ingecotol FC", number: "+573134724602" },
  ]

  return (
    <footer id="contact-section" className="bg-[#0A2342] text-white">
      <div className="max-w-6xl mx-auto px-5 py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">

          {/* Logo y descripción */}
          <div className="flex flex-col items-center">
            <img
              src="/images/Logo blanco.png"
              alt="Ingecotol"
              className="h-28 md:h-32 w-auto mb-4"
            />
            <p className="text-white/60 text-sm text-center">
              Ingeniería y telecomunicaciones. Experiencia, calidad y variedad a su servicio.
            </p>
          </div>

          {/* WhatsApp */}
          <div>
            <h4 className="font-family-oswald text-lg mb-4 text-white/90">WhatsApp</h4>
            <ul className="space-y-2">
              {whatsappContacts.map((contact, index) => (
                <li key={index}>
                  <a
                    href={`https://wa.me/${contact.number.replace(/\+/g, '')}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 text-white/60 hover:text-[#25D366] transition-colors text-sm"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                    </svg>
                    <span>{contact.name}</span>
                    <span className="text-white/40">|</span>
                    <span>{contact.number}</span>
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Correo y horario */}
          <div>
            <h4 className="font-family-oswald text-lg mb-4 text-white/90">Contacto</h4>
            <ul className="space-y-2">
              <li>
                <a
                  href="mailto:ingecotol@hotmail.com"
                  className="flex items-center gap-2 text-white/60 hover:text-[#DC3545] transition-colors text-sm"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <rect width="20" height="16" x="2" y="4" rx="2"/>
                    <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/>
                  </svg>
                  ingecotol@hotmail.com
                </a>
              </li>
              <li className="flex items-center gap-2 text-white/40 text-sm pt-2">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <circle cx="12" cy="12" r="10"/>
                  <polyline points="12 6 12 12 16 14"/>
                </svg>
                Lun - Vie: 8AM - 6PM
              </li>
              <li className="flex items-center gap-2 text-white/40 text-sm">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <circle cx="12" cy="12" r="10"/>
                  <polyline points="12 6 12 12 16 14"/>
                </svg>
                Sábados: 8AM - 1PM
              </li>
            </ul>
          </div>
        </div>

        {/* Línea divisoria y copyright */}
        <div className="border-t border-white/10 mt-8 pt-6 text-center">
          <p className="text-white/40 text-xs">
            &copy; 2025 Ingecotol ltda. Todos los derechos reservados.
          </p>
        </div>
      </div>
    </footer>
  )
}
